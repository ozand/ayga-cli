...
The system acts as a middleware bridge connecting AI Agents (via MCP) and Human operators (via CLI) to the A-Parser engine.
...
graph TD
    ...
    H_Client <-->|REST API| API[A-Parser HTTP Endpoint]
    
    Redis <-->|API::Server::Redis| Engine[A-Parser Engine]
    API <--> Engine
```

## 2. Core Components

### 2.1 CLI Layer (`aparser_cli.main`)
- Framework: **Typer** (FastAPI's sister framework for CLI).
- Handles input parsing, command routing, and interactive prompts.

### 2.2 Transport Router (`aparser_cli.client`)
- Determines the best way to execute a command based on availability and requested mode (`--async` vs `--sync`).
- **Redis Transport:** Uses `redis-py` (async) to push `{"password": "", "action": "request", "data": {...}}` into the `aparser_redis_api` queue.
- **HTTP Transport:** Uses `httpx` to POST JSON to `http://127.0.0.1:9091/API`.

### 2.3 MCP Bridge (`aparser_mcp`)
...
3. MCP Server executes: `aparser redis push SE::Google --queries-file ...`
4. CLI pushes JSON payload to Redis `aparser_redis_api`.
5. CLI immediately returns a `queryId` to the AI agent.
6. A-Parser picks up the job from Redis, processes it using 100 threads.
7. A-Parser pushes results to `aparser_results` queue.
8. A separate Cron job or the AI Agent (via `aparser task wait <queryId>`) pops the result.
