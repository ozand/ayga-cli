"""Manifest cache and fuzzy search for A-Parser parsers.

This module provides:
- ManifestCache: Manages local cache of A-Parser manifest with TTL
...
class Manifest(BaseModel):
    """Complete manifest of all A-Parser parsers."""
...
class ManifestCache:
    """Manages local cache of A-Parser manifest.

    The cache is stored as a compressed JSON file at ~/.config/aparser-cli/manifest.json.gz
    with file permissions set to 600 (owner read/write only).

    Attributes:
        config: AParserConfig instance
    ...
    async def sync(
        ...
        Raises:
            AParserAPIError: If API calls fail
            AParserAuthError: If authentication fails
        """
        # Delay import to avoid circular dependency
        ...
        if verbose:
            print("Syncing parser manifest from A-Parser API...")

        # Fetch from API
        async with AParserHttpClient(self.config) as client:
