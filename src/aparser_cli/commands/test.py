...
def test_cmd(
    ...
    Examples:
        # Basic test
        aparser test FreeAI::Perplexity

        # Test with custom query
        aparser test SE::Google --query "machine learning"

        # Verbose output
        aparser test FreeAI::Perplexity --verbose

        # JSON output
        aparser test FreeAI::Perplexity --json
    """
    config = get_config()
...
def _print_test_failure(test_results: dict, defaults: Optional[Any]) -> None:
    ...
    for step in test_results["steps"]:
        if step["name"] == "validation" and step["status"] == "fail":
            ...
            console.print(f"\n  2. Or see examples: [cyan]aparser run {test_results['parser']} --examples[/cyan]")
