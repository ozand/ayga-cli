"""Preset management commands for A-Parser CLI."""
...
from aparser_cli.presets import get_preset_manager, PresetManager
    ...
    try:
        ...
        if not presets:
            if format_json:
                ...
                console.print("Use [cyan]aparser presets save <name> <parser>[/cyan] to create one.")
            return
...
def save_preset(
    ...
    Examples:
        ...
        aparser presets save perplexity-business "FreeAI::Perplexity" \
            --description "For business queries" \
            --overrides "proxyChecker=reproxy_v4,timeout=120"

        # Save with JSON overrides
        aparser presets save google-search "SE::Google" \
    ...
    try:
        ...
        if format_json:
            ...
            details.extend([
                "",
                "[bold]Usage:[/bold]",
                f"  [cyan]aparser run --preset {preset.name} \"your query\"[/cyan]",
            ])
