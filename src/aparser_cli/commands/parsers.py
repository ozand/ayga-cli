...
app = typer.Typer(help="Manage A-Parser parsers")
console = Console()
...
def parser_info(
    ...
    try:
        ...
        if not manifest and not static_parser:
            ...
            console.print("[yellow]Tip:[/yellow] Use 'aparser parsers list-static' to see available parsers.")
            raise typer.Exit(code=1)
        ...
        if format_json:
            ...
            if required_overrides:
                ...
                example_cmd = f'aparser run {parser_name} "your query" \\\n    --overrides "{",".join(override_parts)}"'
...
def cache_info(
    ...
    if not cache.exists():
        ...
        console.print("Use 'aparser parsers list-static' for offline parser list.")
        return
