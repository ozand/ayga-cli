"""Run command for executing A-Parser with dry-run and pagination support."""
...
from aparser_cli.config import get_config
from aparser_cli.presets import get_preset_manager
...
from aparser_cli.utils.dry_run import DryRunSimulator
from aparser_cli.utils.pagination import execute_with_pagination

app = typer.Typer(
    help="Execute A-Parser requests",
...
def run(
    ...
    saved_preset: Optional[str] = typer.Option(
        ...
        "--saved-preset",
        "-s",
        help="Use saved preset (name from 'aparser presets list')",
    ),
    ...
    Examples:
        # Basic execution
        aparser run SE::Google "machine learning"

        # Use saved preset
        aparser run "What is AI?" --saved-preset perplexity-business

        # Dry-run to preview
        aparser run SE::Google "test" --dry-run

        # With options
        aparser run SE::Google "test" --options "pagecount=5,region=us"

        # Show examples
        aparser run FreeAI::Perplexity --examples

        # Auto-pagination
        aparser run SE::Google "test" --page-all --max-pages 20

        # JSON output
        aparser run SE::Google "test" --json
    """
    config = get_config()
    ...
    if saved_preset:
        ...
        if not preset:
            ...
            if format_json:
                ...
                console.print("Use [cyan]aparser presets list[/cyan] to see available presets.")
            raise typer.Exit(code=1)
    ...
    if not is_valid:
        ...
        if format_json:
            ...
            console.print(f"[red]Error:[/red] {error_msg}")
            console.print(f"\n[yellow]Tip:[/yellow] Use [cyan]aparser run {parser} --examples[/cyan] to see usage examples")
        raise typer.Exit(code=1)
...
async def _execute_single(
    ...
    async with AParserHttpClient(config, timeout=timeout) as client:
