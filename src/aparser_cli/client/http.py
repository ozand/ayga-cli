...
from aparser_cli.utils.error_helper import parse_api_error


class AParserHttpClient:
    """Async HTTP client for A-Parser API.

    This client wraps all A-Parser HTTP API methods using httpx.AsyncClient.
    It handles authentication, error mapping, and provides typed interfaces
    for all API operations.

    Args:
        config: AParserConfig instance with connection settings
        timeout: Request timeout in seconds (overrides config.default_timeout)

    Example:
        >>> config = AParserConfig(password=SecretStr("secret"))
        >>> client = AParserHttpClient(config)
        >>> await client.ping()
        True
    """
    ...
    def __init__(
        self,
        config: Optional[AParserConfig] = None,
    ...
    def _ensure_connected(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            raise AParserHTTPError("Client not connected. Use 'async with' or call connect()")
        return self._client
    ...
    def _build_payload(self, action: str, data: Optional[dict] = None) -> dict:
        ...
        if not password:
            raise AParserAuthError("A-Parser password not configured")
    ...
    async def _request(
        ...
        Raises:
            AParserHTTPError: For HTTP-level errors
            AParserAPIError: For API-level errors in response
            AParserTimeoutError: For timeout errors
            AParserAuthError: For authentication failures
        """
    ...
    async def ping(self) -> bool:
        """Check if A-Parser API is accessible.
    ...
    async def one_request(
        ...
        Raises:
            AParserValidationError: If required parameters are missing
            AParserTimeoutError: If request times out
            AParserAPIError: If API returns an error
        """
        if not parser:
            raise AParserValidationError("Parser name is required")
        if not query:
            raise AParserValidationError("Query is required")
    ...
    async def bulk_request(
        ...
        if not parser:
            raise AParserValidationError("Parser name is required")
        if not queries:
            raise AParserValidationError("At least one query is required")
    ...
    async def get_parser_info(self, parser: str) -> dict:
        ...
        Raises:
            AParserValidationError: If parser name is empty
            AParserHTTPError: If HTTP request fails
            AParserAPIError: If API returns an error
        """
        if not parser:
    ...
    async def get_tasks_list(
        ...
        """Get list of tasks/jobs from A-Parser.
    ...
    async def get_task_info(self, task_id: str) -> dict:
        ...
        Raises:
            AParserValidationError: If task_id is empty
            AParserHTTPError: If HTTP request fails
            AParserAPIError: If API returns an error
        """
        if not task_id:
            raise AParserValidationError("Task ID is required")
