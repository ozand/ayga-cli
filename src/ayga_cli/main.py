"""Main entry point for ayga-parser CLI.
...
from ayga_cli.config import ayga-parserConfig, get_config, reload_config
...
def _patch_click_make_metavar() -> None:
    ...
    def compat_make_metavar(self: click.Parameter, ctx: Optional[click.Context] = None) -> str:
        if ctx is None:
            ctx = click.Context(click.Command("ayga-parser"))
    ...
    def compat_typer_argument_make_metavar(
        ...
        if "ctx" in type_signature.parameters:
            type_var = self.type.get_metavar(self, ctx or click.Context(click.Command("ayga-parser")))
        else:
...
app = typer.Typer(
    name="ayga-parser",
    help="ayga-parser CLI - Redis and HTTP API client for ayga-parser",
...
def version_callback(value: bool) -> None:
    ...
    if value:
        console.print(f"[bold blue]ayga-parser CLI[/bold blue] version [green]{__version__}[/green]")
        raise typer.Exit()
...
def main(
    ...
    """ayga-parser CLI - Redis and HTTP API client for ayga-parser.

    [bold]Quick Start:[/bold]
    [ayga-parser config init]     - Initialize configuration
    [ayga-parser ping]            - Test connection
    [ayga-parser parsers list]    - List available parsers

    [bold]Transports:[/bold]
    • Redis (primary): High-performance queue-based communication
    • HTTP (fallback): Direct synchronous requests

    For more help: [cyan]ayga-parser <command> --help[/cyan]
    """
    pass
...
def config_init(
    ...
    console.print(Panel.fit(
        "[bold blue]ayga-parser CLI Configuration[/bold blue]\n\n"
    ...
    config_content = f"""# ayga-parser CLI Configuration
# Generated by ayga-parser config init
...
"""
    ...
    # Ask about password
    if typer.confirm("Store ayga-parser password in OS keyring?", default=True):
        password = typer.prompt("ayga-parser password", hide_input=True)
...
def config_show() -> None:
    ...
    table = Table(title="ayga-parser Configuration", show_header=True)
    ...
    config_file = ayga-parserConfig.get_config_dir() / "config.yaml"
...
def config_set(
    ...
    if not config_file.exists():
        console.print(f"[red]✗[/red] Config file not found. Run 'ayga-parser config init' first.")
        raise typer.Exit(1)
...
def config_auth(
    password: Optional[str] = typer.Option(
        ...
        "--password",
        "-p",
        help="ayga-parser password (will prompt if not provided)",
    ),
) -> None:
    """Store ayga-parser password in OS keyring."""
    if password is None:
        password = typer.prompt("ayga-parser password", hide_input=True)
